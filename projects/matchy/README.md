...
## Step 2 - Replace
 1. Write a function declaration called `replace` with a signature of `replace(animals, name, replacement) { //... } ` that:
   [x] Takes 3 parameters, an Array of animals, a String representing  the name of an animal on which to perform a search, and an Object that represents the replacement animal.
   [x] If an animal with that name exists within the `animals` Array, replace it's entire Object with the replacement Object.
...
## Step 3 - Remove
 1. Write a function declaration called `remove` with a signature of `remove(animals, name)` that:
   ...
   [x]If an animal with that name exists within the `animals` Array, remove it.
